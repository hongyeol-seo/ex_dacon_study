...
for page in range(200,300):
    ...
    articles = driver.find_elements(By.CLASS_NAME, 'mCBkyc.ynAwRc.MBeuO.nDgy9d')
    print(f'-------------{page}page---------------')
    for i in articles :
        print(i.text)
...
try:
    ...
    df = pd.concat([df, pd.DataFrame(data, columns=['text'])], ignore_index=True)  # 새로운 데이터 추가
...
with pd.ExcelWriter(path, engine='openpyxl', mode='a') as writer:
    df.to_excel(writer, sheet_name='sheet1', index=False, header=not writer.sheets['sheet1'])
